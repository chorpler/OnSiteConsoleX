...
let HARDCLOSE = true;
let SPLASH_WINDOW_MINIMUM_TIMEOUT = 1000;
...
;
let windowTitle = `OnSiteConsoleX ${packageVersion}`;
let testMode = false;
...
const app_icon = electron_1.nativeImage.createFromPath(icon_path);
// let con, win, winUpdate, startupWin, pdfWin, mainWindowState;
let win, winUpdate, splashWindow, randomWindows = [];
let mainWindowState;
function createWindow() {
    ...
    win = new electron_1.BrowserWindow(defaultWindowOptions);
    ...
    let url = getFilePathAsURL(index_location);
    let Args = process.argv.slice(1);
    // let loadDevTools:boolean = false, unFullScreen:boolean = false;
    let loadDevTools = false;
    let unFullScreen = false;
    for (let val of Args) {
...
function createSplashWindow(loadDevTools, dimensions) {
    ...
    let appVersion = electron_1.app.getVersion();
...
function createUpdateStatusWindow(updateVersion, showDevTools) {
    winUpdate = new electron_1.BrowserWindow({
        width: 400,
        height: 400,
        frame: true,
        parent: win,
...
function createRandomWindow() {
    ...
    let randomWin = new electron_1.BrowserWindow(opts);
    ...
    randomWin.on('page-title-updated', (evt, title) => {
        evt.preventDefault();
        log.info(`RandomWindow(${id}): Window title updated to:`, title);
    });
    randomWin.on('close', (evt) => {
        evt.preventDefault();
        let thiswin = evt && evt.sender && evt.sender instanceof electron_1.BrowserWindow ? evt.sender : randomWin;
...
function hardClose(browserwindow) {
    if (browserwindow instanceof electron_1.BrowserWindow && !browserwindow.isDestroyed()) {
...
function closeRandomWindow(randomWin) {
    ...
    // let winID:number;
    // let count:number = randomWindows.length;
    if (randomWin instanceof electron_1.BrowserWindow) {
        let count = randomWindows.length;
        let id = randomWin.id;
...
electron_updater_1.autoUpdater.on('update-downloaded', (ev, info) => {
    ...
    let msg = "Updated version available";
    if (info) {
        if (typeof info.releaseNotes === 'string') {
            msg = info.releaseNotes;
        }
        else if (Array.isArray(info.releaseNotes)) {
            let count = info.releaseNotes.length;
            let note = info.releaseNotes[count - 1];
            if (note.note) {
                msg = note.note;
            }
        }
    ...
    electron_1.dialog.showMessageBox(dialogOpts, (response, checked) => {
        closeUpdateWindow();
        if (response === 0) {
...
electron_1.ipcMain.on('manual-check-update', (event) => {
    log.info(`IPC: Received manual-check-update event, event:`, event);
    electron_updater_1.autoUpdater.checkForUpdates().then((res) => {
...
function showAlert(title, text, detail) {
    ...
    electron_1.dialog.showMessageBox(win, dialogOpts, (response, checked) => {
...
function showDialog(dialogOpts) {
    ...
    electron_1.dialog.showMessageBox(win, opts, (response, checked) => {
...
electron_1.ipcMain.on('dialog-show', (event, dialogOpts) => {
...
electron_1.ipcMain.on('kill-update-status-window', (event, options) => {
    log.info(`IPC: Got kill-update-status-window event, options:`, options);
    closeUpdateWindow();
});
electron_1.ipcMain.on('show-update-status-window', (event, options) => {
...
electron_1.ipcMain.on('show-about', (event, options) => {
    log.info(`IPC: Got show-about event, options: `, options);
    try {
        let thisAppVersion = electron_1.app.getVersion();
...
electron_1.ipcMain.on('exit-app', (event, options) => {
    log.info(`IPC: got exit-app event, options:\n`, options);
    // app.quit();
    quitApp();
});
electron_1.ipcMain.on('window-console', (event, windowConsole) => {
    log.info(`IPC: got window-console event with options:\n`, windowConsole);
    // con = windowConsole;
});
electron_1.ipcMain.on('show-pdf', (event, options) => {
...
electron_1.ipcMain.on('show-splash', (event, options) => {
...
electron_1.ipcMain.on('hide-splash', () => {
    log.info(`IPC: got hide-splash event`);
    closeSplashWindow();
});
electron_1.ipcMain.on('show-update', (event, appInfo) => {
...
electron_1.ipcMain.on('hide-update', () => {
    log.info(`IPC: got hide-update event`);
    closeUpdateWindow();
});
electron_1.ipcMain.on('send-update-error', (event, text) => {
...
electron_1.ipcMain.on('progress-message', (event, progressObject) => {
    log.info(`IPC: got progress-message event: `, progressObject);
    if (winUpdate && winUpdate.webContents) {
        winUpdate.webContents.send('progress-message', progressObject);
    }
});
electron_1.ipcMain.on('random-window-create', (event, data) => {
    log.info(`IPC: got random-window-create event`);
    createRandomWindow();
});
electron_1.ipcMain.on('random-window-close', (event, data) => {
...
electron_1.ipcMain.on('random-window-close-all', (event, data) => {
...
// @ts-ignore
electron_1.app.on('dev-exit-app', () => {
...
//# sourceMappingURL=electronLoad.js.map