...
@Component({
  selector: 'invoice-logistics',
  templateUrl: 'invoice-logistics.html',
})
export class InvoiceLogisticsComponent implements OnInit,OnDestroy {
  ...
  @Input('sorts') sorts:number[] = [-1, -1, -1, -1, -1, -1];
  ...
  constructor(
    ...
    window["onsitelogisticsinvoicecomponent" ] = this;
    window["onsitelogisticsinvoicecomponent2"] = this;
  }

  ngOnInit() {
    Log.l('InvoiceLogisticsComponent: ngOnInit() fired');
  ...
  ngOnDestroy() {
    Log.l("InvoiceLogisticsComponent: ngOnDestroy() fired");
  ...
  public installSubscribers() {
    this.siteSubscription = this.dispatch.invoiceSiteUpdated().subscribe((siteObject: any) => {
      Log.l("InvoiceLogisticsComponent: received site Observable update!\n", siteObject.site);
    ...
    this.reportsSubscription = this.dispatch.invoiceReportsUpdated().subscribe((reportsObject: any) => {
      Log.l("InvoiceLogisticsComponent: received reports Observable update!\n", reportsObject.reports);
  ...
  public testInvoice() {
    ...
    for(let i = 0; i < count; i++) {
      ...
      // let unit  = ~~(Math.random()*(umax - umin)) + umin;
      // let wo    = ~~(Math.random()*(wmax - wmin)) + wmin;
      // let hours = ~~(Math.random()*(hmax-hmin)) + hmin;
      // let mins  = ~~(Math.random()*(4 - 0)) + 0;
      let unit  = 88888888;
      let wo    = 999999999;
      let hours = 5;
      let mins  = 2;
