...
@Component({
  selector: 'report-driving-view',
  templateUrl: 'report-driving-view.html',
})
export class ReportDrivingView implements OnInit,OnDestroy {
  ...
  @Input('report')   report : ReportDriving              ;
  @Input('reports') reports : ReportDriving[]  = []      ;
  ...
  public title      : string         = "View Driving Report" ;
  public visible    : boolean        = true               ;
  public dialogLeft : number         = 250                ;
  public dialogTop  : number         = 100                ;
  public header     : string         = "View Driving Report"      ;
  ...
  ngOnInit() {
    Log.l("ReportDrivingView: ngOnInit() called");
  ...
  ngOnDestroy() {
    Log.l("ReportDrivingView: ngOnDestroy() fired.");
  ...
  public runWhenReady() {
    ...
    let rpt = this.report || this.reports[0];
  ...
  public createMenuLists() {
    let rpt:ReportDriving = this.report;
  ...
  public updateDisplay(report?:ReportDriving) {
  //   let rpt:ReportDriving = report || this.report;
  //   // let reportDate = moment(rpt.report_date, "YYYY-MM-DD");
  //   // let reportDate = moment(rpt.report_date, "YYYY-MM-DD");
  //   let hours = rpt.getTotalTime();
  ...
  public async saveClicked(event?:any) {
    ...
    try {
      ...
      //   }
      // }
      spinnerID = await this.alert.showSpinnerPromise("Saving driving report …");
      let res = await this.db.saveReportDriving(rpt, username);
      ...
      await this.alert.hideSpinnerPromise(spinnerID);
      // this.notify.addError("ERROR", `Error saving report: '${err.message}'`, 5000);
      await this.alert.showErrorMessage("ERROR", "Error saving driving report", err);
    }
  }

  public async deleteReport(event:any) {
    let spinnerID;
    let report:ReportDriving = this.report;
    try {
      ...
      if(confirm) {
        let newReport:ReportDriving = report.clone();
        window['onsitedeletedreports'].push(report);
        let res:any = await this.db.deleteDrivingReport(report);
  ...
  //     let rpt1:ReportDriving = this.reports.splice(idx, 1)[0];
