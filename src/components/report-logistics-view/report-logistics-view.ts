...
@Component({
  selector: 'report-logistics-view',
  templateUrl: 'report-logistics-view.html',
})
export class ReportLogisticsViewComponent implements OnInit,OnDestroy {
  ...
  public reportUndo  :ReportLogistics[] = []              ;
  ...
  constructor(
    ...
    window['onsitelogisticsreportview']  = this;
    window['onsitelogisticsreportview2'] = this;
    window['p'] = this;
    window['_dedupe'] = _dedupe;
  }

  ngOnInit() {
    Log.l("ReportLogisticsViewComponent: ngOnInit() called");
    if(this.data.isAppReady()) {
      let backupReport:ReportLogistics = oo.clone(this.report);
  ...
  ngOnDestroy() {
    Log.l("ReportLogisticsViewComponent: ngOnDestroy() fired.");
  ...
  public installSubscribers() {
    this.keySubscription = this.keyService.commands.subscribe((command:Command) => {
      switch(command.name) {
        case "ReportLogisticsView.previous" : this.previous(); break;
        case "ReportLogisticsView.next"     : this.next(); break;
        case "ReportLogisticsView.previous" : this.previous(); break;
        case "ReportLogisticsView.next"     : this.next(); break;
      }
    });
  ...
  public setHeader(idx?:number) {
    ...
    this.header = `View Logistics Report (${index+1} / ${count})`;
    return this.header;
  }
  ...
  public async saveNoExit(event?:Event):Promise<any> {
    try {
      let res:any = await this.db.saveReportLogistics(this.report);
      if(this.period) {
      ...
      Log.l(`saveNoExit(): Error saving logistics report without exiting`);
      Log.e(err);
      this.notify.addError("ERROR", `Error saving logistics report: '${err.message}'`, 10000);
    }
  }
  ...
  public async deleteReport(event:Event) {
    ...
    try {
      ...
      let text:string = `Are you sure you want to delete this logistics report? This will permanently delete this logistics report and cannot be undone.`;
      if(this.data.isDeveloper) {
        text += "<br>\n<br>\n(However, just between a couple of developers, I'm saving the logistics report to the onsitedeletedlogisticsreports array, so you can recover it from the DevTools.)";
      }
      window['onsitedeletedlogisticsreports'] = window['onsitedeletedlogisticsreports'] || [];
      let confirm:boolean = await this.alert.showConfirmYesNo(title, text);
      if(confirm) {
        let newReport:ReportLogistics = report.clone();
        window['onsitedeletedlogisticsreports'].push(report);
      ...
      Log.e(err);
      this.notify.addError("ERROR", `Error deleting logistics report ${report._id}: '${err.message}'`, 10000);
