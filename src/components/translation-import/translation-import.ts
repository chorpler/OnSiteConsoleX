...
@Component({
  selector: 'translation-import',
  templateUrl: 'translation-import.html',
  // styleUrls: ['./translation-import.scss'],
})
export class TranslationImport implements OnInit,OnDestroy {
  @Input('imports') imports:TranslationTable;
  ...
  public title         : string = "Translation Imports"        ;
  ...
  public header        : string         = "Translation Imports";
  ...
  public selectedImports : TranslationTable = []               ;
  ...
  public cols = [
    ...
    { header: 'English'          , field: 'en'         , }, 
    { header: 'Spanish'          , field: 'es'         , }, 
    { header: 'Maintenance Type' , field: 'maint_type' , }, 
  ];
  ...
  constructor(
    ...
    window['onsiteTranslationImport']  = this;
    // window['onsitemessages2'] = this;
    window['p2'] = this;
  }

  ngOnInit() {
    Log.l('TranslationImport: ngOnInit() fired');
  ...
  ngOnDestroy() {
    Log.l('TranslationImport: ngOnDestroy() fired');
  ...
  public async runWhenReady() {
    try {
      ...
      Log.l("TranslationImport.runWhenReady(): Error loading translation editor!");
      Log.e(err);
  ...
  public initializeSubscriptions() {
    ...
    this.textSub = this.modelChanged.pipe(debounceTime(this.debounceTime), distinctUntilChanged()).subscribe(data => {
      Log.l(`TranslationImport: Model changed and debounce time elapsed. Data:`, data);
    });
  }
  ...
  public textChanged(key:TranslationTableRecordKey, value:string, element?:any, event?:any) {
    // Log.l(`TranslationImport.textChanged(): Key:`, key);
    // Log.l(`TranslationImport.textChanged(): Value:`, value);
    // Log.l(`TranslationImport.textChanged(): Element:`, element);
    // Log.l(`TranslationImport.textChanged(): Event:`, event);
    // window['onsiteelement1'] = element;
    this.modelChanged.next(value);
    // if(this.timer) {
    //   this.timer.clearTimer();
    // }
    // // Log.l(`TranslationImport.textChanged(): Event is:`, event);
    // this.timer = moment.duration(this.debounceTime).timer(() => {
    //   // Log.l(`TranslationImport.textChanged(): Timeout, running function. Event is:`, event);
  ...
  //   Log.l(`TranslationImport.updateMaintenanceType(): type is '${type}', event is:`, event);
  //   if(!type) {
  //     Log.l(`TranslationImport.updateMaintenanceType(): Type was null.`);
  //     // let idx1 = this.allTranslations.indexOf(this.record);
  //     let idx = this.maintTranslations.indexOf(this.record);
  //     if(idx > -1) {
  //       Log.l(`TranslationImport.updateMaintenanceType(): translation has to be removed from maintenance translations`);
  //       this.maintTranslations.splice(idx, 1);
  //     }
  //   } else {
  //     Log.l(`TranslationImport.updateMaintenanceType(): Type was NOT null.`);
  //     let idx2 = this.maintTranslations.indexOf(this.record);
  //     if(idx2 === -1) {
  //       Log.l(`TranslationImport.updateMaintenanceType(): translation has to be added to maintenance translations`);
  ...
  //   try {
  //     Log.l(`TranslationImport.addTranslation(): Called …`);
  ...
  //     // return res;
  //   } catch(err) {
  //     Log.l(`TranslationImport.addTranslation(): Error adding new translation`);
  //     Log.e(err);
  //     throw err;
  //   }
  // }
  ...
  //   try {
  //     Log.l(`TranslationImport.deleteTranslation(): Called …`);
  ...
  //       } catch(err) {
  //         Log.l(`TranslationImport.deleteTranslation(): Error saving deletion`);
  //         Log.e(err);
  ...
  //     // return res;
  //   } catch(err) {
  //     Log.l(`TranslationImport.deleteTranslation(): Error deleting translation`);
  //     Log.e(err);
  //     throw err;
  //   }
  // }
  ...
  //     Log.l(`TranslationImport.showOptions(): Called …`);

  //     // return res;
  //   } catch(err) {
  //     Log.l(`TranslationImport.showOptions(): Error showing options for Translation Editor page`);
  //     Log.e(err);
  //     throw err;
  //   }
  // }
  ...
  //     }
  //   } catch(err) {
  //     Log.l(`TranslationImport.previous(): Error during move to previous record`);
  //     Log.e(err);
  //     throw err;
  //   }
  // }
  ...
  //     }
  //   } catch(err) {
  //     Log.l(`TranslationImport.next(): Error during move to next record`);
  //     Log.e(err);
  //     throw err;
  //   }
  // }
  ...
  public async dropdownVisible(status:boolean, evt?:any):Promise<boolean> {
    try {
      Log.l(`TranslationImport.dropdownVisible(): State '${status}', called with event:`, evt);
      if(status) {
        this.dropdownActive = status;
      } else {
        await this.data.delay(250);
        this.dropdownActive = status;
      }
      return this.dropdownActive;
    } catch(err) {
      Log.l(`TranslationImport.dropdownVisible(): Error checking dropdown visible status`);
      Log.e(err);
      throw err;
    }
  }

  public async updateRowMaintenanceType(row:TranslationTableRecord, evt?:any):Promise<any> {
    try {
      Log.l(`TranslationImport.updateRowMaintenanceType(): Called for row with event:\n`, row, evt);
      if(!row.maint_type) {
        delete row.maint_type;
      }
      // return res;
    } catch(err) {
      Log.l(`TranslationImport.updateRowMaintenanceType(): Error updating row maintenance type for row:`, row);
      Log.e(err);
      throw err;
    }
  }

  public isDuplicate(row:TranslationTableRecord):boolean {
    let key = row.key;
    let dupes = this.translations.filter(rcrd => rcrd && rcrd.key && rcrd.key === key);
    if(dupes.length) {
      return true;
    }
    return false;
  ...
  public isDuplicateImport(row:TranslationTableRecord):boolean {
    let key = row.key;
    let dupes = this.imports.filter(rcrd => rcrd && rcrd.key && rcrd.key === key);
    if(dupes.length > 1) {
      return true;
    }
    return false;
  ...
  public overwriteRecord(src:TranslationTableRecord, dest:TranslationTableRecord, table?:TranslationTable):TranslationTableRecord {
    if(src && dest && src !== dest && typeof src === 'object' && typeof dest === 'object') {
      ...
      let text = `TranslationImport.cancelClicked(): overwriteRecord(): Must provide two object parameters. Invalid parameter(s)`;
      Log.w(text + ":", src, dest);
  ...
  public async cancelClicked(evt?:Event):Promise<any> {
    try {
      ...
      Log.l(`TranslationImport.cancelClicked(): Error during cancel of translation edit`);
      Log.e(err);
      throw err;
    }
  }
  ...
  public async saveClicked(evt?:Event):Promise<any> {
    ...
    try {
      ...
      let saveRows = this.selectedImports;
      ...
      this.close.emit(saveRows);
      // return res;
    } catch(err) {
      Log.l(`TranslationImport.saveClicked(): Error during save of translation edit`);
      Log.e(err);
      await this.alert.hideSpinnerPromise(spinnerID);
      throw err;
    }
  ...
  public async saveTranslations(evt?:Event):Promise<any> {
    try {
      ...
      Log.l(`TranslationImport.saveTranslations(): Error during save of translation edit`);
      Log.e(err);
      throw err;
    }
  }
