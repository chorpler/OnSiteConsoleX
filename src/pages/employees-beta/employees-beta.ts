...
import { SelectItem, MenuItem, MultiSelect,                          } from 'primeng/primeng'            ;
...
export class EmployeesBetaPage implements OnInit,OnDestroy {
  ...
  public generateFieldList() {
    ...
    fields.push({ field: "salaried"       , header: "Salaried"    , filter: true , filterPlaceholder: "Salaried"    , order: 22 }); // 21
    fields.push({ field: "created"        , header: "Created"     , filter: true , filterPlaceholder: "Created"     , order: 23 }); // 22
    this.colOpts = [];
    let initialize = [];
  ...
  public resetEmployeeTable(evt?:any) {
    ...
    let dt:DataTable;
    if(this.dt && this.dt instanceof DataTable) {
      dt = this.dt;
      dt.reset();
    }
  }
