...
export class WorkSitesPage implements OnInit,OnDestroy {
  ...
  public async reorderSites(evt:any):Promise<any> {
    try {
      ...
      this.jobsites = sites;
      // let res:Array<Jobsite> = await this.saveSortOrder();
      // return res;
  ...
  public orderColumns(event?:any):Array<any> {
  ...
  public async saveSortOrder(evt?:any):Promise<Array<Jobsite>> {
    try {
      Log.l(`saveSortOrder(): Attempting to save site list:\n`, this.dt.value);
      let siteList:Array<Jobsite> = this.dt.value;
      let updatedSites:Array<Jobsite> = await this.db.saveSites(siteList);
